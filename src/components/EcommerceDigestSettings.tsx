...
import { Bell, BellOff, Hash, Mail, Loader2 } from "lucide-react";
...
export function EcommerceDigestSettings({ amId, children }: { amId: string; children?: React.ReactNode }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!validation?.valid) {
        throw new Error(validation?.error || "Slack cíl nelze ověřit");
      }
    ...
    onSuccess: () => {
      ...
      toast({ description: "Nastavení digestu uloženo" });
      setOpen(false);
    },
    onError: (err: Error) => {
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({ description: `Testovací zpráva odeslána (${data?.sent || 0} klientů)` });
    },
    onError: (err: Error) => {
      toast({ variant: "destructive", description: err.message });
    },
